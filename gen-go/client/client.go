...
// WagClient is used to make requests to the breakdown service.
...
func New(basePath string, logger wcl.WagClientLogger, transport *http.RoundTripper) *WagClient {
	...
	circuit := &circuitBreakerDoer{
		...
		circuitName: fmt.Sprintf("breakdown-%s", shortHash(basePath)),
		logger:      logger,
	}
	circuit.init()
...
// the three env vars: SERVICE_BREAKDOWN_HTTP_(HOST/PORT/PROTO) to be set. Otherwise it returns an error.
// The logger provided should be specifically created for this wag client. If tracing is required,
// provide an instrumented transport using the wag clientconfig module. If no tracing is required, pass nil to use
// the default transport.
func NewFromDiscovery(logger wcl.WagClientLogger, transport *http.RoundTripper) (*WagClient, error) {
	url, err := discovery.URL("breakdown", "default")
	if err != nil {
		url, err = discovery.URL("breakdown", "http") // Added fallback to maintain reverse compatibility
		if err != nil {
			return nil, err
		}
	}
...
// 500: *models.InternalError
// default: client side HTTP errors, for example: context.DeadlineExceeded.
func (c *WagClient) PostDeploy(ctx context.Context, i *models.Deploys) error {
	headers := make(map[string]string)

	var body []byte
	path := c.basePath + "/v1/deploy"
	...
	return c.doPostDeployRequest(ctx, req, headers)
}

func (c *WagClient) doPostDeployRequest(ctx context.Context, req *http.Request, headers map[string]string) error {
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Canonical-Resource", "postDeploy")
	req.Header.Set(VersionHeader, Version)
	...
	// Add the opname for doers like tracing
	ctx = context.WithValue(ctx, opNameCtx{}, "postDeploy")
	req = req.WithContext(ctx)
	// Don't add the timeout in a "doer" because we don't want to call "defer.cancel()"
	// until we've finished all the processing of the request object. Otherwise we'll cancel
	// our own request before we've finished it.
...
// 500: *models.InternalError
// default: client side HTTP errors, for example: context.DeadlineExceeded.
func (c *WagClient) PostUpload(ctx context.Context, i *models.RepoCommit) error {
	headers := make(map[string]string)
